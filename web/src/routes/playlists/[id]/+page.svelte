<script lang="ts">
    ...
    import { playlistDetailCache, playlistsCache } from "$lib/stores/pageData.svelte";
    ...
    async function saveEdit() {
        ...
        try {
            ...
            playlistDetailCache.set(playlist.id, { ...playlist, name: editName });
    ...
    async function handleCoverUpload(e: Event) {
        ...
        try {
            ...
            playlistDetailCache.set(playlist.id, { ...playlist, cover_path: result.cover_path });
            playlistsCache.invalidate();
            await invalidate('app:playlists');
    ...
    async function deletePlaylist() {
        ...
        try {
            ...
            playlistDetailCache.invalidate(playlist.id);
            playlistsCache.invalidate();
            await invalidate('app:playlists');
...
<div class="p-6">
    <div class="flex gap-6 mb-8">
        <div class="relative group">
            {#if coverUrl}
                ...
                />
            {:else}
                <div class="w-56 h-56 rounded-lg bg-gradient-to-br from-zinc-700 to-zinc-800 flex items-center justify-center shadow-xl">
        ...
        <div class="flex flex-col justify-end">
            <p class="text-sm text-zinc-400 uppercase tracking-wider">Playlist</p>
            {#if editing}
                <form
                    onsubmit={(e) => {
                        e.preventDefault();
                ...
                >
                    ...
                    />
                    <button type="submit" class="text-emerald-400 text-sm">Save</button>
                    <button
                        type="button"
            ...
            {:else}
                <h1 class="text-5xl font-bold mt-2 mb-4">{playlist.name}</h1>
            {/if}
            ...
            <div class="text-sm text-zinc-400">
                {songs.length} songs
                {#if playlist.public}
                    <span class="ml-2 px-2 py-0.5 bg-emerald-500/20 text-emerald-400 rounded text-xs">Public</span>
                {/if}
            </div>
        </div>
    </div>
    ...
    <div class="flex items-center gap-4 mb-6">
        <button
            ...
            class="w-14 h-14 bg-emerald-500 rounded-full flex items-center justify-center hover:scale-105 shadow-lg disabled:opacity-50"
        >
    ...
    {#if songs.length > 0}
        <div class="border-b border-zinc-800 pb-2 mb-2 flex items-center text-xs text-zinc-500 uppercase tracking-wider px-3">
